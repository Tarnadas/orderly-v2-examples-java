...
public class PnL {
   ...
   public JSONObject settlePnL() throws InvalidKeyException, SignatureException, OrderlyClientException, IOException {
      ...
      JSONObject settlePnlMessage = new JSONObject();
      settlePnlMessage.put("brokerId", config.brokerId);
      settlePnlMessage.put("chainId", config.chainId);
      settlePnlMessage.put("timestamp", Instant.now().toEpochMilli());
      settlePnlMessage.put("settleNonce", settleNonce);
